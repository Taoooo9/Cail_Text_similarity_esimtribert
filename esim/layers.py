...
class RNNSenDropout(nn.Dropout):
    """
    Dropout layer for the inputs of RNNs.
...
class Seq2SeqSenEncoder(nn.Module):
    """
    RNN taking variable length padded sequences of vectors as input and
    encoding them into padded sequences of vectors of the same length.
    ...
    def __init__(self,
        ...
        super(Seq2SeqSenEncoder, self).__init__()
        ...
        self._encoder1 = rnn_type(hidden_size * 2,
                                 hidden_size,
                                 num_layers=num_layers,
                                 bias=bias,
                                 batch_first=True,
    ...
    def forward(self, sequences_batch, sen_per_length, sequences_lengths):
        ...
        sequences_batch = sequences_batch.view(batch_size * max_sen_num, -1, dim)
        packed_batch = nn.utils.rnn.pack_padded_sequence(sequences_batch,
                                                         sen_per_length,
                                                         batch_first=True,
                                                         enforce_sorted=False)
