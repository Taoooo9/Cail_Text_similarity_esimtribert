...
def get_sen_mask(sequences_batch, sequences_lengths):
    """
    Get the mask for a batch of padded variable length sequences.
    ...
    """
    batch_size = sequences_batch.size()[0]
    max_sen_len = sequences_batch.size()[-1]
    max_length = sequences_batch.size()[1]
    mask = torch.ones(batch_size, max_length, max_sen_len, dtype=torch.float)
